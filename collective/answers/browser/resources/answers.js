...
// collective.answers. If you don't see the js here executing, double check
...
jq(document).bind('loadInsideOverlay', function() {
    ...
    jq("div#answer-action-buttons-container button.collective-answers-add-answer-button")
...
jq(document).ready(function(){
    ...
    jq("input#collective-answers-question-add-button").click(function(event) {
        event.preventDefault();
    ...
    jq("button.collective-answers-add-answer-button").prepOverlay({
        subtype: 'ajax',
        filter: '#content',
...
function updateQuestions(data, textStatus, jqXHR) {
    ...
    jq('div#answers-container').append(html);
